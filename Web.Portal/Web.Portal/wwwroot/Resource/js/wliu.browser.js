...
function browserDetect(userAgent) {
    ...
    var browserObj = detector.detect();
    browserObj.name = ("" + browserObj.name).charAt(0).toUpperCase() + ("" + browserObj.name).slice(1).toLowerCase();
    browserObj.platform = ("" + browserObj.os.split(" ")[0]).trim();
    browserObj.platformVersion = parseFloat(("" + browserObj.os.split(" ")[1]).trim()).toFixed(1);
    browserObj.versionNumber = parseFloat(("" + browserObj.versionNumber).trim()).toFixed(1);
    return browserObj;
}

/* below code only for typescript  import module 
...
export var RecommendList = {
    "Chrome": {
        browser: "Chrome 58+",
        platform: ["Android 4.1+", "iOS 11+", "MacOS 10+", "Windows 7+"]
    }
}
export var BrowserAgent = browserDetect();
*/