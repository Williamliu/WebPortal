...
WLIU.Database = function (database) {
    if ($.isPlainObject(database)) {
        ...
        this.state = database.state || 0;
        this.lang = database.lang || "en";
...
WLIU.Database.prototype = {
    ...
    Get: function (url) {
        ...
        if (this.getUrl !== "") {
            this.state = 1;
            this.method = "get";
            var ndb = new WLIU.GDatabase(this);

            let defer = $.Deferred();
            AJAX.Post(this.getUrl, ndb).then(data => {
                if (data.other && $.isPlainObject(data.other)) {
                    for (let o in data.other) {
                        self.other[o] = data.other[o];
                    }
                }

                if (data.tables && $.isPlainObject(data.tables)) {
                    for (let tname in data.tables) {
                        self.tables[tname].navi = new WLIU.Navi(data.tables[tname].navi);
                        self.tables[tname].AddORows(data.tables[tname].rows);
                        self.tables[tname].state = 0;
                    }
                }
                self.state = 0;
                this.DBAjaxErrorHandle();
                defer.resolve(self);
            }).catch(data => {
                self.state = 0;
...
WLIU.Table.prototype = {
    ...
    CurrentIndex: function () {
        return $.indexInList(this.rows, { guid: this.navi.guid });
    },
    CurrentKey: function () {
    ...
    Reload: function (url, pageNo) {
        ...
        let ntable = new WLIU.GTable(this);
    ...
    Scan: function (url) {
        this.scanUrl = url || this.scanUrl;
        //prepare post table
        this.method = "get";
        this.navi.isLoading = 1;
        let ntable = new WLIU.GTable(this);
        ntable.navi.pageNo = 1;
        let defer = $.Deferred();
        var self = this;
        $("div[wliu][loading][method='get']").loading("show");
        self.TableAjax(self.scanUrl, ntable).then(data => {
            self.state = 0;
            self.navi.isLoading = 0;
            self.SyncGetTable(data);
            self.TableAjaxErrorHandle();
    ...
    Empty: function (guid, kvs) {
        if (this.GuidRow(guid)) {
            if (this.GuidRow(guid).error.HasError() === false) {
                this.EmptyRemove(guid);
    ...
    SyncSaveTable: function (gtable) {
        ...
        if (gtable && $.isArray(gtable.rows)) {
            ...
            for (let rowIdx = 0; rowIdx < gtable.rows.length; rowIdx++) {
                ...
                if (ogrow && row) {
                    if (ogrow.error.hasError) {  // important: pay attention to gtable error.hasError ,  table error.HasError() 
                        row.error = new WLIU.Error(ogrow.error);
                    }
                    switch (row.state) {
                        case 0:
                            break;
                        case 1:
                            if (ogrow.columns) {
                                for (let colName in ogrow.columns) {
                                    ...
                                    if (ogcol && col) {
                                        if (ogcol.error.hasError) {
                                            col.error = new WLIU.Error(ogcol.error);
                                        }
                                        else
                                        {
                                            col.state = 0;
...
WLIU.Navi = function (navi) {
    if ($.isPlainObject(navi)) {
        ...
        this.isLoading = navi.isLoading || 0;
        this.guid = navi.guid || "";
...
WLIU.Row.prototype = {
    ...
    ChangeState: function () {    // Row Error combine from  Columns Error 
        ...
        this.error.Reset();
        for (let colName in this.columns) {
            if (this.columns[colName].error.HasError()) {
                this.error.code = Math.max(this.columns[colName].error.code, this.error.code);
                this.error.messages.push(this.columns[colName].error.Message());
            }
        }
...
WLIU.GDatabase = function (database) {
    if (database)
    {
        this.other = database.other;
        this.AddTables(database.tables);
    }
    else
    {
        this.other = {};
...
WLIU.GDatabase.prototype = {
    AddTable: function (table) {
        if (table) {
            if (table.state === 1)
                this.tables[table.name] = new WLIU.GTable(table);
        }
        return this;
    },
    AddTables: function (tables) {
...
WLIU.GTable.prototype = {
    AddRow: function (row) {
        if (row.error.HasError()===false && row.state>0)
            this.rows.push(new WLIU.GRow(row));
        return this;
    },
    AddRows: function (rows) {
        this.rows = [];
    ...
    CloneNavi: function (table) {
        if (table && table.navi) {
            ...
            this.navi.current       = table.navi.current;
    ...
    AddColumn: function (column) {
        switch (this.state) {
            case 0:
                break;
            case 1:
                if ( this.errors[column.name] && this.errors[column.name].HasError() === false && column.state === 1) {
                    this.columns[column.name] = new WLIU.GColumn(column);
                }
                break;
            case 2:
                this.columns[column.name] = new WLIU.GColumn(column);
...
var AJAX = {
    ...
    Call: function (url, method, data, callback) {
        let promiseA = new Promise((resolutionFunc, rejectionFunc) => {
            $.ajax({
                dataType: 'json',
                data: JSON.stringify(data),
                /* disable progress if performance issue
                xhr: function () {
                ...
                */
                ...
                error: function (xhr, tStatus, errorTh) {
                    ...
                    switch (resp.status) {
                        case 401:
                        case 404:
                            resp.data.error.message = Words(resp.data.error.message);
                            resp.data.error.messages[0] = Words(resp.data.error.messages[0]);
