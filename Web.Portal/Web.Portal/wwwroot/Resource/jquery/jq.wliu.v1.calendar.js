    ...
    _eventChange: function () {
        ...
        this._calendar.Get().then(d => {
            ...
            for (let idx in d.events) {
                ...
                if (evtObj) {
                    evtObj.AddToCalender(self.element);
                }
            }
...
WLIU.Event.prototype = {
    AddToCalender: function (container) {
        let timestr = this.from ? this.from.hhmm() : "";
        timestr += (timestr ? "~" : "") + this.to.hhmm();
        let time = `${this.dateNo > 0 ? this.dateNo+") " : ""}<span>${timestr}</span>`;

        let ctooltip = [
            'tooltip-toggle="hover" ',
            `tooltip-body="${timestr} ${this.title}" `,
            'tooltip-target="wliuToolTip" ',
            'tooltip-placement="down" '
        ].join('');

        let dateStatus = 0;
        if (this.status) dateStatus = this.state;

        let subject = "";
        if (this.subject !== "") subject = `<b>${this.subject}</b><br>`;

        let style = "";
        if (this.color !== "") style = `style="color:${this.color};"`;

        let dateItem_html = `<li class="date-item status-${dateStatus}" ${style} eid="${this.id}" ${ctooltip}>${subject}${time} ${this.title}</li>`;
        $(`td.date>div.date-event[yy=${this.yy}][mm=${this.mm}][dd=${this.dd}]`, container).append(dateItem_html);
    }
};
